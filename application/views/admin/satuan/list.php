...
function hapus(id) {
    bootbox.confirm("Apakah Anda Yakin Menghapus Data Ini?", function(result) {
        if(result) {
            window.location.href = "<?php echo site_url() . '/admin/satuan/delete' ?>/"+id;
        }
    }); 
}
...
function ubah(kode) {
    ...
    $.ajax({
        url: "<?php echo site_url() . '/admin/satuan/edit/'; ?>" + kode,
            ...
            <tbody>
                ...
                ?>
                    <tr class="">
                        ...
                        <td align="center">
                            <button type="button" class="btn btn-success btn-xs" data-toggle="modal" href="#editModal" onclick="ubah('<?php echo $v['kode_satuan'] ?>')" >
                                <i class="icon-edit"></i>
                            </button>
                        </td>
                        <td align="center">
                            <button type="button" class="btn btn-danger btn-xs" onclick="hapus('<?php echo $v['kode_satuan'] ?>')">
